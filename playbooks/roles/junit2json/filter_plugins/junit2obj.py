...
version_added: "2.0.0"
short_description: >
  Transforms JUnit XML report text data into an identically structured JSON object.
description: >
  This filter plugin transforms the JUnit XML data to JSON representation of it.
  Special care is taken to ensure the counters are integers, durations (`time` attribute) are floats
  and timestamps are parseable so that the object is analysis friendly.
  Unlike previous implementations (`redhatci.ocp.junit2dict`).
...
def _collect_failure(testcase_elem: etree.Element) -> Optional[Dict[str, Any]]:
    ...
    result_dict = {"message": failure_elem.get("message", "")}
...
def _collect_error(testcase_elem: etree.Element) -> Optional[Dict[str, Any]]:
    ...
    result_dict = {"message": error_elem.get("message", "")}
...
def _process_test_case(testcase_elem: etree.Element) -> Dict[str, Any]:
    ...
    collector_handlers: Tuple[
        Callable[[etree.Element], Optional[Dict[str, Any]]], ...
    ] = (
        _collect_failure,
        _collect_error,
        _collect_skipped,
...
class FilterModule:
    ...
    def junit2obj(self, junit_report_text: str) -> Dict[str, Any]:
        ...
        test_suites = [
            _process_test_suite(testsuite_elem) for testsuite_elem in elements
        ]
        ...
        for attr in set(ATTR2CONVERSION.keys()) - {"name", "timestamp"}:
            ...
            if xml_value is None:
                ...
                report[attr] = sum(
                    converter(suite.get(attr, None), default=_default)
