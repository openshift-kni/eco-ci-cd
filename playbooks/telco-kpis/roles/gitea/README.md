...
│   ├── HTTP: http://bastion.example.com:3000/
│   ├── SSH: ssh://git@bastion.example.com:2222/
...
1. Bastion `host_vars` vault (e.g., `bastion-host`)
2. `ansible_group_bastions` vault
3. `ansible_group_all` vault

**Expected Vault Variables**:
- `ansible_user`: Bastion username (typically `admin`)
...
| `gitea_domain` | `{{ ansible_fqdn }}` or `bastion.example.com` | Gitea domain |
...
**Web UI**: http://bastion.example.com:3000/admin/telco-kpis-reports

**Git Clone**:
```bash
# HTTPS (requires credentials)
git clone http://bastion.example.com:3000/admin/telco-kpis-reports.git

# SSH (requires SSH key)
git clone ssh://git@bastion.example.com:2222/admin/telco-kpis-reports.git
```
...
2. Check vault path for your bastion host in vault
...
gitea_repo_http_url: >-
  http://{{ gitea_admin_user }}:{{ gitea_admin_password }}@{{
  gitea_domain }}:{{ gitea_http_port }}/{{
  gitea_admin_user }}/{{ gitea_repo_name }}.git
```
...
ssh bastion "ls -l ~/telco-kpis-artifacts/spree-02/"
# Compare with node-info timestamp
ssh bastion "jq -r '.collected_at' ~/telco-kpis-artifacts/spree-02/node-info-spree-02.json"
```
